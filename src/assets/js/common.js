    // // 定义一些公共的属性和方法
    // generateOptions() {
    //   var params = this.publicMethod.cityInfo;
    //   console.log(params);
    //   //生成Cascader级联数据
    //   var result = [];
    ...
    //         label: params[i].value,
    //         value: params[i].value,
    //       };
    //       if (params[i].children) {
    //         parent.children = this.getchilds(params[i].children); //获取子节点
    //       }
    ...
    //     if (arr[item].value) {
    //       childs.push({
    //         label: arr[item].value,
    //         value: arr[item].value,
    //       });
    //     }
    //     if (arr[item].children) {
    //       if (arr[item].children.length > 0) {
    //         let childscopy = this.getchilds(arr[item].children); //递归获取子节点
    //         if (childscopy.length > 0) {
    //           childs[item].children = childscopy;
    //         }
    //       }
    //     }
    //   }
    ...
    function getUrluserAndUser() {
      ...
      var params = queryInfo.split("=")[1];
      if (params.indexOf("&") === -1) {
        var url_userId = params
      } else {
        var url_userId = params.split('&')[0];
      }
      var userId = window.sessionStorage.getItem("userId");
      return [url_userId, userId];
    }

    function getUrlParamsObj(urlStr) {
      if (typeof urlStr == "undefined") {
        var url = "?" + window.location.href.split("?")[1];
      } else {
        var url = "?" + urlStr.split("?")[1];
      }
      var theRequestParams = new Object();
      if (url.indexOf("?") != -1) {
        var str = url.substr(1);
        var strs = str.split("&");
        for (var i = 0; i < strs.length; i++) {
          theRequestParams[strs[i].split("=")[0]] = decodeURI(strs[i].split("=")[1]);
        }
      }
